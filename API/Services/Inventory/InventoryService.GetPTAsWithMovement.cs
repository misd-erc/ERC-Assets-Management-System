...
namespace API.Services.Inventory
{
    ...
    {
public async Task<IActionResult> GetPTAsWithMovement([FromQuery] PTAPaginationQueryParams model)
        {
            ...
            try
            {
                ...
                // Get all non-deleted movements so we can both identify which PTAs have
                // movement history and read each item's condition off its latest movement.
                var allMovements = await _getTools.PTA
                    .GetTblPTAMovements(context)
                    .Where(x => x.PTAId.HasValue)
                    .ToListAsync();

                var idsWithMovementsSet = new HashSet<long>(allMovements.Select(x => x.PTAId!.Value));

                var latestConditionByPtaId = allMovements
                    .GroupBy(x => x.PTAId!.Value)
                    .ToDictionary(
                        g => g.Key,
                        g => g.OrderByDescending(x => x.CreatedAt).ThenByDescending(x => x.Id).First().Remarks);

                // Keep only those that have been issued at least once (have movement history)
                ...
                foreach (var x in ptaList)
                {
                    var ptaModel = new PTAResponseModel
                    {
                        ...
                        Condition = latestConditionByPtaId.TryGetValue(x.Id, out var condition) ? condition : null,
                ...
                await AuditTrailTool.LogActivityAsync(_options, "Viewed PTAs with movements", actionBy: model.ActionBySystemUserId);
                ...
                return Ok(ApiResponse<PTAResponseModel>.OkPaginated(
                    ...
                    model.PageSize,
                    totalCount,
                    "PTAs with movements retrieved successfully"
                ));
            }
            catch (Exception ex)
            {

...
export class UnifiedAssetService {
  ...
  static async getAll(filters?: {
    ...
    try {
      ...
      // Category filter value is now the numeric ID as a string (or 'all')
      const categoryId =
      ...
      if (groupFilter === 'ppe') {
        ...
        allItems = [
          ...(ppeResponse.items || []).map(item => this.mapApiToUnifiedAsset(item, 'PPE')),
          ...(seResponse.items || []).map(item => this.mapApiToUnifiedAsset(item, 'SE')),
        ];
  ...
  static async batchUpload(file: File, userId: string, sessionKey: string, onProgress?: (percent: number) => void): Promise<{ success: boolean; code: string; message: string; data: unknown }> {
